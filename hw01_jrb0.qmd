...
We can then select the month and day from these 10 entries and place them in their own data set, then semi_join the original data set with the newly created list of 10 days in order to filter out all non top-ten entries:

```{r, echo=TRUE, include=TRUE}

top10_days <- group_by(flights_planes, month, day) %>%
  summarize(
    median_delay = median(dep_delay, na.rm=T),
    .groups = "drop"
  ) %>%
...
flights_top10 <- flights_planes %>%
  semi_join(top10_days, by = c("month", "day"))
